...
[ -n "${YOMI_NEW_GROUP}" ] && {
	# App group IDs
	sed -i "" "s/group.${YOMI_ORIG_GROUP}.app/group.${YOMI_NEW_GROUP}.app/g" "ios/Yomi Share/Yomi Share.entitlements"
	sed -i "" "s/group.${YOMI_ORIG_GROUP}.app/group.${YOMI_NEW_GROUP}.app/g" "ios/Runner/Runner.entitlements"
	sed -i "" "s/group.${YOMI_ORIG_GROUP}.app/group.${YOMI_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
	# Bundle identifiers
	sed -i "" "s/${YOMI_ORIG_GROUP}.app/${YOMI_NEW_GROUP}.app/g" "ios/Runner.xcodeproj/project.pbxproj"
}

[ -n "${YOMI_NEW_TEAM}" ] && {
	# Code signing team
	sed -i "" "s/${YOMI_ORIG_TEAM}/${YOMI_NEW_TEAM}/g" "ios/Runner.xcodeproj/project.pbxproj"
}
cat << EOHELP
...
2. Fix it yourself
    ...
    - Ensure that Xcode created the App ID successfully (for yomi.app and yomi.app.Yomi-Share)
...
[ -n "${YOMI_INSTALL_IPA}" ] && {
  ...
  cfgutil --foreach install-app "${TMPDIR}/yomi.ipa"
  rm -rf "${TMPDIR}"
}
